...
public class AboutActivity extends AppCompatActivity {
    ...
    private static final String GITHUB_RELEASES_URL = "https://api.github.com/repos/jnetai-clawbot/DeadMansSwitch/releases/latest";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        try {
            ...
            tvAbout.setText("Dead Man's Switch\n\nScheduled check-ins with emergency actions. If you don't check in, it triggers alerts.\n\nMade by jnetai.com");
                        ...
                        new Handler(Looper.getMainLooper()).post(() -> {
                            ...
                            if (latestTag != null && !latestTag.isEmpty()) {
                                if (!latestTag.equals(currentVersion)) {
                                    ...
                                    Toast.makeText(AboutActivity.this, "You are up to date! (v" + currentVersion + ")", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Toast.makeText(AboutActivity.this, "Could not determine latest version", Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else {
                        new Handler(Looper.getMainLooper()).post(() -> {
                            btnCheckUpdate.setEnabled(true);
                            btnCheckUpdate.setText("Check for Update");
                            Toast.makeText(AboutActivity.this, "Update check failed (HTTP " + responseCode + ")", Toast.LENGTH_SHORT).show();
                        });
                    }
                    conn.disconnect();
                } catch (Exception e) {
    ...
    private void shareApp() {
        try {
            ...
            shareIntent.putExtra(Intent.EXTRA_SUBJECT, "Dead Man's Switch");
            shareIntent.putExtra(Intent.EXTRA_TEXT,
                "Check out Dead Man's Switch - Scheduled check-ins with emergency actions!\n\n" +
                "https://github.com/jnetai-clawbot/DeadMansSwitch/releases\n\nMade by jnetai.com");
